...
bt_product1 = InlineKeyboardButton(text=f'{products[0][1]}', callback_data='product_buying')
bt_product2 = InlineKeyboardButton(text=f'{products[1][1]}', callback_data='product_buying')
bt_product3 = InlineKeyboardButton(text=f'{products[2][1]}', callback_data='product_buying')
bt_product4 = InlineKeyboardButton(text=f'{products[3][1]}', callback_data='product_buying')
...
async def get_buying_list(message):
    for i in range(4):
        with open(f'{products[i][4]}', 'rb') as img:
            await message.answer_photo(img,
                f'Название: {products[i][1]} ' +
...
# Объявляю класс состояний для регистрации пользователя
...
# Обработка сообщения с именем электронной почтой
...
async def set_age(message, state):
    ...
    if message.text.isdigit():
        ...
        await message.answer('Неверное значение. Введите свой возраст в целых числах:')
        await RegistrationState.age.set()
